...
/// # Ok::<(), std::io::Error>(())
pub trait Write {
    /// Write a buffer into this writer with a specified style, returning how many bytes were
    /// written.
    ///
    ...
    /// one attempt to write to any wrapped object, plus whatever is needed to change the style.
    ///
    /// Calls to `write` are not guaranteed to block waiting for data to be written, and a write
    /// which would otherwise block can be indicated through an [`Err`] variant.
    ///
    /// If the return value is `Ok(n)` then it must be guaranteed that `n <= buf.len()`. A return
    /// value of `0` typically means that the underlying object is no longer able to accept bytes
    /// and will likely not be able to in the future as well, or that the buffer provided is empty.
    ///
    /// # Errors
    ///
    /// Each call to `write` may generate an I/O error indicating that the operation could not be
    /// completed. If an error is returned then no bytes in the buffer were written to this writer,
    /// but the writers state may have changed to match the new style.
    ///
    /// It is not considered an error if the entire buffer could not be written to this writer.
    ///
    /// An error of the [`ErrorKind::Interrupted`] kind is non-fatal and the write operation should
    ...
    /// Attempts to write an entire buffer into this writer using a specified style.
    ///
    /// This method will continuously call [`write`] until there is no more data to be written or
    /// an error of non-[`ErrorKind::Interrupted`] kind is returned. This method will not return
    /// until the entire buffer has been successfully written or such an error occurs.  The first
    /// error that is not of [`ErrorKind::Interrupted`] kind generated from this method will be
    /// returned.
    ///
    ...
    ///
    /// This method is primarily used to interface with the [`stylish::format_args!`] macro, but it
    /// is rare that this should explicitly be called. The [`stylish::write!`] macro should be
    /// favored to invoke this method instead.
    ///
    /// This function internally uses the [`write_all`](Write::write_all) method on this trait and
    /// hence will continuously write data so long as no errors are received. This also means that
